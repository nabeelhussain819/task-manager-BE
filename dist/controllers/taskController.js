...
const createTask = asyncHandler(async (req, res) => {
    ...
    const task = await taskService.createTask(req.user.userId, title, description, checklist);
...
const updateChecklistItem = asyncHandler(async (req, res) => {
    ...
    const task = await taskService.updateChecklistItem(taskId, req.user.userId, parseInt(itemIndex), completed);
    res.json(task);
});
